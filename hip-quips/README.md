...
The two most useful "measurements" of curveu in my opinion are called UNIT and UNITLEN in the primuvconvert() VEX function's docs. UNIT (alternatively called the curve's parameterization or parametric coordinates) is measured based on each point's number, eg. if a curve has 5 points, the 3rd point would have a curveu of 0.5 (1st: 0, 2nd: 0.25, 3rd: 0.5, 4th: 0.75, 5th: 1). UNITLEN is measured based on the arc length of the curve, so a point that is exactly halfway along the length of the curve would have a curveu of 0.5.

When it comes to comparing the two, if a curve has evenly spaced points eg. from a resample SOP, UNIT and UNITLEN "curveu"s are almost identical (with small decimal place differences). However, if the curve is nonuniform in its distribution of points, UNITLEN and UNIT will be different. UNIT coordinates are used for intrinsic/parametric coordinates and are expected as the input for functions such as primuv(). However, UNITLEN coordinates in my experience are often desirable when taking advantage of the attribute as a user, as many effects benefit from taking into account the actual arc length of the curve without getting disrupted by the density/sparsity of points. Just make sure you are giving functions and nodes the type of curveu that they expect (you can convert between coordinate systems using the primuvconvert() VEX function)!
